        ...
        case "heretic":
            #for member in SERVER_DATA['server'].members:
               # if EXTRA_ROLES['possessed'] in member.roles:
                ...
                msgText = f"{usr.mention} cast Heretic Rig but as a result they ended up 1% less welcome in this society."
                # msgCounting = await SEND(ch,"You cast Heretic Rig but as a result you ended up 1% less welcome in this society."
            
        case "wicked":
            msgText = f"{usr.mention} cast Wicked Rig and destroyed all the nasty stairs!"
...
async def CastRig(rigPick, ch, usr):
    ...
    if rigPick == "chameleon":
        ...
        while True:
            ...
            if randomAttempts == 3:
                await SEND(ch, "Wow. You really broke me. Shocking, since I am suppossedly the epitome of creation, but.. alas.")
                # chameleonSuccess = False
